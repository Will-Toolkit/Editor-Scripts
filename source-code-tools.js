...
(function () {
    ...
    async function addRedesignLink() {
        async function getRedesignURL() {
            let pagePromise = new Promise(function (resolve) {
                var doc;
                var xhttp = new XMLHttpRequest();
                xhttp.onreadystatechange = function () {
                    if (this.readyState == 4 && this.status == 200) {
                        doc = document.implementation.createHTMLDocument();
                        doc.documentElement.innerHTML = xhttp.response;
                        resolve(doc.querySelector("#ctl00_cTitle_hlViewRedesign")?.href || "");
                    }
                };
                xhttp.open("GET", "https://www.toolkit.uk/pages", true);
                xhttp.send();
            });

            let output = await pagePromise;
            return output;
        }

        let redesignURL = await getRedesignURL();
        if (redesignURL) {
            const redesignButton = document.createElement("a");
            const viewSiteButton = document.querySelector("#ctl00_cphSimple_ctl00_hlViewWebsite")
            viewSiteButton.before(redesignButton);
            const alias = viewSiteButton.href.matchAll(/^(?:[^\/]*\/){3}(.*)$/g).next().value[1]
            const redesignPage = redesignURL + alias;
            redesignButton.outerHTML = `<a id="ctl00_cphSimple_ctl00_hlViewRedesign" class="bootstrapeditor-el bootstrapeditor-btn bootstrapeditor-btn-vw" href="${redesignPage}" target="_blank">View Redesign ›</a>`
            if (alias.endsWith('-redesign')) {
                viewSiteButton.style.display = "none";
            }
        }
    }
